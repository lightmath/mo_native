var loadingView = (function () {
    function loadingView() {
        ...
        if (this.sOS == "Conch-ios") {
            ...
            this.bridge = PlatformClass.createClass("mo.JSBridge");
        }
    }
...
if (nMem <= 524288) {
    conchConfig.atlasNum = 30;//15张 每张为1024*1024
}
else if (nMem > 524288 && nMem <= 1048576) {
    conchConfig.atlasNum = 45;//20张 每张为1024*1024
}
else if (nMem > 1048576) {
     conchConfig.atlasNum = 60;//30张 每张为1024*1024
}
...
//    window.loadingView.setTips([""]);//设置tips数组，会随机出现
    window.loadingView.showTextInfo = false;
}
